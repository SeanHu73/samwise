...
type SyncPayload = {
  results: Array<{ operationId: string; ok: boolean }>;
...
export async function queueOperation(
  operation: Omit<SyncOperation, "operationId" | "clientCreatedAt" | "status">,
) {
  await db.outbox.add({
    ...operation,
    operationId: id(),
...
async function performSync() {
  ...
  try {
    ...
    return failed.length
      ? { ok: false, reason: "operation_failed", count: succeeded.length }
      : { ok: true, count: succeeded.length };
  } catch {
    if (pending.length)
      await db.outbox.bulkUpdate(
        pending.map((item) => ({
  ...
  await db.transaction(
    ...
    async () => {
      for (const change of payload.changes) {
        ...
        if (table) await table.put(entity);
        for (const field of change.conflicts ?? []) {
          await db.conflicts.put({
            id: `${change.entityType}:${String(entity.id)}:${field}:${change.sequence}`,
            entityType: change.entityType,
            entityId: String(entity.id),
            field,
            localValue: "Preserved on this device",
            remoteValue: (entity as Record<string, unknown>)[field],
            createdAt: now(),
          });
        }
      }
...
function tableFor(type: SyncEntityType) {
  return (
    {
      task: db.tasks,
      project: db.projects,
      task_event: db.taskEvents,
      area: db.areas,
      direction: db.directions,
      goal: db.goals,
      milestone: db.milestones,
      note: db.notes,
      attachment: db.attachments,
      planning_session: db.planningSessions,
      planning_profile: db.planningProfiles,
      calendar_event: db.calendarEvents,
      agent_run: db.agentRuns,
    } as Record<SyncEntityType, { put: (value: unknown) => Promise<unknown> }>
  )[type];
}
